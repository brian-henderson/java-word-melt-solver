...
class Location implements Comparable<Location> {
  ...
  Location nextNeighbor() {  
    ...
    StringBuilder tempWord = new StringBuilder(this.word);
	
    switch(iterationMode) {
    case CHANGE_LETTER:
    	  String nextLetterTemp = String.valueOf(nextLetter);
    	  tempWord.replace(this.indexToChange, this.indexToChange+1, nextLetterTemp);
    	  temp.word = tempWord.toString();
    ...
    case INSERT_LETTER:
    	  tempWord.insert(this.indexToChange, this.nextLetter);
    	  temp.word = tempWord.toString();
    	  if(this.nextLetter == 'z') {
        if (this.word.length() > this.indexToChange ) {
          ...
          this.indexToChange = 0;
          this.iterationMode = (this.word.length() < 2 ) ? DONE : DELETE_LETTER;
        }
    	  }
    	  else {
        this.nextLetter++;
    ...
    case DELETE_LETTER:
    	  tempWord.deleteCharAt(indexToChange);
    	  temp.word = tempWord.toString();
    	  if (this.indexToChange < this.word.length() - 1) {
        this.indexToChange ++;
    	  }
  ...
  boolean isLess(Location loc) {
    int temp = this.word.compareTo(loc.word);
    return temp > 0;
  }
